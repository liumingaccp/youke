...
public class TbConstans {
    ...
    static {
        ...
        TB_ORDER_STATUS.put("TRADE_CLOSED_BY_TAOBAO",6);
    }

    /**
     * good(好评),neutral(中评),bad(差评)
     */
    public static Map<String, Integer> TB_RTADE_RATE = new HashMap<>();
    static {
        TB_RTADE_RATE.put("good",1);
        TB_RTADE_RATE.put("neutral",2);
        TB_RTADE_RATE.put("bad",3);
    }

    public static DefaultTaobaoClient getDefaultTaobaoClient(){
        DefaultTaobaoClient client = new DefaultTaobaoClient(URL, OPENAPPKEY, OPENAPPSECRET);
        return client;
    }

    public static SecurityClient getSecurityClient(){
        DefaultTaobaoClient client = new DefaultTaobaoClient(URL, OPENAPPKEY, OPENAPPSECRET);
        return new SecurityClient(client, "GKGSJsdZ0b1bm0tfTdVwvS6IKgWUr6NobE/EymGjVzQ=");
    }
}
